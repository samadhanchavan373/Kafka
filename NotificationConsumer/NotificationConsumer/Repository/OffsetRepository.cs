...
namespace NotificationConsumer1.Repository
{
    public class OffsetRepository
    {
        ...
        public async Task SaveOffsetAsNextToConsume(ConsumeResult<string, string> cr, string consumerGroup)
        {
            using var connection = new SqlConnection(_connectionString);
            await connection.OpenAsync();
            using var transaction = connection.BeginTransaction();

            long nextOffset = cr.Offset.Value + 1;
            ...
            try
            {
                ...
                upsertCmd.Parameters.AddWithValue("@Topic", cr.Topic);
                upsertCmd.Parameters.AddWithValue("@PartitionId", cr.Partition.Value);
                upsertCmd.Parameters.AddWithValue("@LastOffset", nextOffset);
                await upsertCmd.ExecuteNonQueryAsync();
